...
quatro eixos principais, apresentados na ordem em que fundamentam
conceitualmente o sistema: o modelo de avaliação de risco integrado,
que define \emph{o que} é medido e como o risco é quantificado; as
fontes de dados meteorológicos externas que alimentam esse modelo; a
plataforma de \textit{software} que implementa o modelo e disponibiliza
seus resultados; e, por fim, o protótipo de \textit{hardware} embarcado
que realiza a coleta física dos dados em campo. Ao final do capítulo,
descreve-se ainda a metodologia adotada para os testes de validação do
sistema, cujos resultados são detalhados no Capítulo~\ref{cap:resultados}.
...
inteligência artificial em trabalhos acadêmicos, registra-se que
ferramentas de IA generativa foram utilizadas em duas frentes deste
trabalho: (i) auxílio à depuração e ao desenvolvimento do código-fonte
do \textit{firmware}, do \textit{backend} e do \textit{frontend} do
sistema; e (ii) revisão textual e de formatação deste documento. Em
ambos os casos, a autoria intelectual do modelo de risco, das decisões
de arquitetura e das análises apresentadas é da autora.
...
O modelo de avaliação de risco integrado foi desenvolvido pela
autora com base na abordagem multidimensional documentada por
\citeonline{rico2008mine} para causas de ruptura em barragens de
rejeitos, que identificaram que 39\% dos 147 casos
de ruptura de barragens de rejeitos documentados resultaram da combinação de múltiplos fatores, e não de uma causa
isolada. 

Partindo dessa premissa, o modelo quantifica, em cada
ciclo de análise, as condições geotécnicas e meteorológicas da
estrutura monitorada por meio de um índice numérico adimensional.
O fator de risco integrado ($F_{risco}$) é calculado pela soma
ponderada de sete variáveis, conforme a Equação~\ref{eq:frisco}.
...
e meteorológico, descrita nas subseções seguintes. A escolha das sete
variáveis reflete os dois grupos de fatores historicamente associados
a rupturas de barragens de rejeitos: condições geotécnicas de
saturação do maciço e condições meteorológicas de precipitação e
pressão atmosférica, ambos discutidos nos casos de Merriespruit e
Brumadinho apresentados no \autoref{cap1Revisao}
\cite{rico2008mine, robertson2019feijaoson2019}.
...
solo medida pelo higrômetro, seguindo o mesmo princípio funcional dos
piezômetros convencionais descritos no \autoref{cap2Fundamentacao},
que também inferem o comportamento hidrostático da barragem a partir
de uma grandeza física medida localmente \cite{machado2007}. O fator
de lençol freático ($F_{len}$) é calculado pela Equação~\ref{eq:flencol}:
...
O valor crítico de referência de 25\% é um limiar técnico definido
pela autora. Ele indica o ponto em que o solo é considerado em estado de
saturação elevada, a partir da faixa operacional de calibração do sensor
higrômetro resistivo FC-28 (Seção~\ref{subsec:higrometro}) --- e não
de um valor extraído da literatura de barragens ---, com $F_{len}$
limitado ao intervalo $[0;\,1]$. 

O peso de 0,40 atribuído a
esta variável, o maior do modelo, foi definido pela autora por ser o
indicador mais direto da estabilidade do maciço --- escolha amparada pelo comportamento observado em Brumadinho
(2019), onde o nível d'água interno persistentemente elevado
antecedeu o colapso \cite{robertson2019feijaoson2019}. A variável componente é obtida pela
Equação~\ref{eq:vlen}:
...
ativando o LED vermelho e o \textit{buzzer}. A adoção de um gatilho
de ruptura direto, desacoplado do cálculo ponderado, é coerente com o
registrado por \citeonline{rico2008mine}, para quem a saturação
extrema do maciço antecede diretamente a perda de resistência ao
cisalhamento em rupturas por liquefação.
...
circular de dez leituras consecutivas. O tamanho do \textit{buffer}
foi definido pela autora como solução de compromisso entre a
resolução temporal necessária para capturar variações relevantes de
umidade e as limitações de memória RAM do microcontrolador ESP8266
(Seção~\ref{subsec:esp8266}). O valor normalizado é obtido pela
Equação~\ref{eq:vtaxa}:
...
O peso de 0,10 atribuído a $V_{taxa}$ foi arbitrado pela autora: não há, na
literatura consultada, um coeficiente numérico específico para a
velocidade de saturação em modelos de baixo custo, mas sua inclusão se
justifica pela relevância da rapidez do fenômeno nos mecanismos de
ruptura por liquefação, nos quais a perda de resistência ao
cisalhamento ocorre de forma súbita \cite{arnez1999avaliacao}.
...
histórica dos dados da estação D6594. A inspeção do histórico da estação, tendo por base as
requisições e respostas da API BNDMET/DECEA reproduzidas no
\autoref{anexo:bndmet} e o cálculo detalhado apresentado no
\autoref{apendice:vch30d}, identificou, em cada uma das duas
temporadas chuvosas analisadas, uma janela de 30 dias consecutivos
com acúmulo elevado: 346\,mm entre 2 e 31 de janeiro de 2026 e
310\,mm entre 5 de janeiro e 3 de fevereiro de 2025. 

O valor de
300\,mm representa, portanto, um
limiar próximo aos maiores acúmulos observados nessas duas janelas
analisadas, e não o resultado de uma varredura exaustiva de todas as
janelas móveis de 30 dias já registradas pela estação.

Diferentemente dos limiares de 24 horas e de
7 dias, extraídos de referências internacionais específicas
\cite{rico2008mine, sistemaalertario2025}, este limiar foi definido
pela autora a partir da análise de dados históricos regionais, na
ausência de um valor de referência bibliográfica consolidado para essa
janela temporal em barragens de rejeitos. O componente é calculado pela
...
OWM e classificada pela autora em cinco categorias de intensidade,
inspiradas na nomenclatura de classificação de chuva adotada pelo
Sistema Alerta Rio --- fraca, moderada, forte, muito forte e pancada
de chuva \cite{sistemaalertario2025}. É importante ressaltar que a
classificação original do Alerta Rio refere-se à intensidade
\textbf{instantânea} da chuva, medida em mm/h
\cite{sistemaalertario2025}. 

A autora adaptou apenas os
\textbf{nomes} das cinco categorias do Alerta Rio, redefinindo os
intervalos numéricos e atribuindo um fator discreto ($F_{int}$)
próprio a cada classe, conforme a Tabela~\ref{tab:intensidade}. Os
valores de corte (5, 25, 50 e 80\,mm) e os fatores correspondentes
não constam da fonte original e resultam de arbítrio da autora, por
analogia proporcional às faixas do Alerta Rio.
...
A queda da pressão atmosférica é um indicador reconhecido da
aproximação de frentes frias e de sistemas de baixa pressão
associados a precipitações intensas: o ar quente e úmido em
ascensão reduz a pressão à superfície, favorecendo a formação de
nuvens e chuva \cite{noaa2024pressure}.

Por esse
motivo, a tendência de pressão em janelas de três horas é um
parâmetro sinótico padronizado internacionalmente --- reportado
pelas estações meteorológicas conforme a Tabela de Código 0200 da
\textit{World Meteorological Organization} (WMO) --- exatamente para
caracterizar a evolução da pressão no curto prazo e antecipar a
chegada de sistemas de mau tempo \cite{wmo2014guide}.

A escolha do campo \texttt{grnd\_level} (pressão atmosférica ao
nível do solo), com \texttt{pressure} (pressão ao nível do mar)
como valor de \textit{fallback} quando o primeiro está ausente,
decorre diretamente da disponibilidade desses dados na resposta
padrão do \textit{endpoint} \texttt{/weather} consultado pelo
sistema (Seção~\ref{subsec:owm}).

O limiar de 5\,hPa em 3 horas e o peso de
0,05 atribuído a esta variável --- o menor entre os sete componentes
--- foram adotados pela autora como indicador auxiliar e antecipatório
de instabilidade atmosférica iminente, e não como parâmetro geotécnico
direto; optou-se por um peso reduzido justamente por se tratar de um
sinal indireto e de menor especificidade para a estabilidade do
maciço, sem correspondência quantitativa direta na literatura
consultada sobre rupturas de barragens de rejeitos. O componente
...
variáveis do modelo. A distribuição reflete a abordagem
multidimensional recomendada por \citeonline{rico2008mine}: o maior
peso (0,40) foi atribuído à variável geotécnica direta (umidade do
solo), enquanto os pesos meteorológicos (que somam 0,45) refletem
a relevância das causas pluviométricas identificadas por esses
autores em 25\% dos casos mundiais analisados. Os pesos
remanescentes ($V_{taxa}=0{,}10$ e $V_{press}=0{,}05$) foram
arbitrados pela autora como componentes complementares de menor
magnitude, dado o caráter indireto desses indicadores em relação à
ruptura estrutural. Os pesos somam 1,00, garantindo que $F_{risco}$
...
na Tabela~\ref{tab:alertas}. Os limiares (0,45 e 0,75) foram
arbitrados pela autora seguindo a lógica de bandas de risco em três
níveis já adotada pelo Sistema Alerta Rio \cite{sistemaalertario2025}
e pela metodologia de Categoria de Risco da ANM, que também
classifica barragens em faixas discretas de risco a partir de
pontuações agregadas \cite{brasil2017portaria70389}. A opção por um
corte próximo à metade inferior da escala para o nível VERDE
($\leq 0{,}45$) reflete uma postura conservadora, priorizando a
antecipação de alertas em detrimento de eventuais falsos positivos.
...
apresentadas nas Figuras~\ref{fig:uc-parte1} a~\ref{fig:uc-parte4}.

% ══════════════════════════════════════════════════════════════
\paragraph*{Parte 1 --- Sistema Embarcado (ESP8266)}\hfill\break
% ══════════════════════════════════════════════════════════════
Esta parte, ilustrada na Figura~\ref{fig:uc-parte1}, reúne os casos de
uso relativos ao dispositivo embarcado.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]
...
\begin{enumerate}
  ...
  \item O \textit{firmware} serializa os dados em JSON com
        ...
        \path{POST /api/sensor/dados} com cabeçalho
...
% ══════════════════════════════════════════════════════════════
Esta parte, ilustrada na Figura~\ref{fig:uc-parte2}, reúne os casos de
uso relativos ao cadastro público e à autenticação do administrador.
...
\path{POST /auth/cadastro-basico} sem autenticação.
...
\begin{enumerate}
  ...
  \item O \textit{frontend} envia
        \path{POST /auth/cadastro-basico} com
...
\begin{enumerate}
  ...
  \item O \textit{frontend} envia
        \path{POST /auth/login} com \texttt{\{email, senha\}}.
...
\begin{enumerate}
  ...
  \item O \textit{frontend} envia
        \path{POST /auth/logout} com o token no cabeçalho
...
\begin{enumerate}
  ...
  \item O \textit{frontend} envia
        \path{POST /auth/solicitar-reset-senha}
        com \texttt{\{email\}}.
...
\begin{enumerate}
  ...
  \item O \textit{frontend} envia
        \path{POST /auth/resetar-senha} com
...
% ══════════════════════════════════════════════════════════════
Esta parte, ilustrada na Figura~\ref{fig:uc-parte3}, reúne os casos de
uso relativos ao monitoramento em tempo real e à central de alertas.
...
\begin{enumerate}
  ...
  \item O sistema organiza e exibe os resultados em seis
        ...
        Componentes da Equação~\ref{eq:frisco}, (4) Qualidade
...
\begin{enumerate}
  ...
  \item O sistema constrói o documento HTML completo
        ...
        Componentes da Equação~\ref{eq:frisco} com valores por
        variável; (4) Qua-lidade Técnica do Sistema com
...
\begin{enumerate}
  ...
  \item O ator seleciona um \textit{template} pré-definido
        ...
        ``Enviar Alerta''.
  \item O \textit{frontend} envia
        \path{POST /auth/enviar-alerta} com token JWT.
...
% ══════════════════════════════════════════════════════════════
Esta parte, ilustrada na Figura~\ref{fig:uc-parte4}, reúne os casos de
uso relativos à gestão de usuários básicos, administradores e às
funcionalidades exclusivas do \textit{super\_admin}.
...
\begin{enumerate}
  ...
  \item O ator preenche os campos e clica em
        ``Criar Usuário''.
  \item O \textit{frontend} envia
        \path{POST /auth/cadastro-basico} com token JWT.
...
\begin{enumerate}
  ...
  \item O \textit{frontend} envia
        \path{POST /auth/cadastro-admin} com token JWT.
  \item O \textit{backend} verifica unicidade do e-mail,
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 1 — Modelos de Dados}
% ─────────────────────────────────────────────────────────────
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image54_diagrama_classes_parte1.pdf}}
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 2 — Modelos de Autenticação e Usuários}
% ─────────────────────────────────────────────────────────────
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image54_diagrama_classes_parte2.pdf}}
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 3 — Camada de Serviços e Controladores}
% ─────────────────────────────────────────────────────────────
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/image54_diagrama_classes_parte3.pdf}}
...
de campo booleano que corrige uma inconsistência identificada no firmware
...
campos específicos do firmware, como \texttt{bndmet\_inicializado} e
...
gerenciamento de usuários, o serviço reúne três métodos específicos
...
\texttt{enviarResetSenha()} envia ao usuário um e-mail com
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 4 — Sistema Embarcado}
% ─────────────────────────────────────────────────────────────
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image54_diagrama_classes_parte4.pdf}}
...
normalizado ($F_{len\c{c}ol} = umidade / UMIDADE\_CRITICA$, conforme a
Equação~\ref{eq:flencol}) e uma \textit{flag} indicando se o sensor está operacional. A estrutura
...
resultado completo do cálculo da Equação~\ref{eq:frisco}. Além do fator de risco integrado
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 1 — Boot e Inicialização do ESP8266}
% ─────────────────────────────────────────────────────────────
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_1a.pdf}}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_1b.pdf}}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_1c.pdf}}
...
firmware consulta o endpoint \path{GET /api/sensor/status} — via HTTP simples sem TLS, pois ESP8266 e servidor encontram-se
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_1d.pdf}}
...
a cada 30 min. Um modo de teste com envio fixo a cada 10\,s permanece
disponível no código-fonte, porém comentado na versão em uso.

\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_1e.pdf}}
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 2 — Ciclo de Coleta, Análise de Risco e Envio}
% ─────────────────────────────────────────────────────────────
...
0, 1)$, conforme a Equação~\ref{eq:flencol}. A variável \texttt{sensorOK} é definida como
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_2a.pdf}}
...
(Alberto Flores, ANA). Um mecanismo de \textit{retry} acelerado reduz o intervalo de consulta
para 30 segundos quando \texttt{apiDisponivel = false}, acelerando a recuperação após
falhas temporárias da API sem aguardar o intervalo normal de 5 minutos. Adicionalmente,
uma guarda de sincronização NTP impede que a consulta seja realizada quando
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_2b.pdf}}
...
Tabela~\ref{tab:intensidade}: Fraca ($<5$ mm, fator 0,00), Moderada (5–25 mm, fator 0,25), Forte
(25–50 mm, fator 0,50), Muito Forte (50–80 mm, fator 0,75) e Pancada de Chuva
($\geq 80$ mm, fator 1,00).

\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_2c.pdf}}
  \caption{Seq. 2c — Consulta OpenWeatherMap}
  \label{fig:seq_2c}
  \fonte{Autora, 2026.}
\end{figure}

A Figura~\ref{fig:seq_2d} detalha o cálculo da análise de risco pela Equação~\ref{eq:frisco}.
...
(ajuste que substituiu uma normalização anterior por divisão por 10\%, a qual saturava
o componente em eventos extremos). Quando $F_{len\c{c}ol} \geq 0{,}70$ (equivalente a umidade $\geq 17{,}5\%$)
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_2d.pdf}}
  \caption{Seq. 2d — Cálculo da Análise de Risco (Equação~\ref{eq:frisco})}
  \label{fig:seq_2d}
  \fonte{Autora, 2026.}
\end{figure}

A Figura~\ref{fig:seq_2e} mostra o envio do \textit{payload} JSON ao \textit{backend} 
via \path{POST /api/sensor/dados} com \textit{timeout} de 15.000 milissegundos. O
...
(VERMELHO) — com o modo de teste de 10\,s fixo disponível no código-fonte, porém comentado na versão em uso.
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth]{figures/image55_seq_2e.pdf}}
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 3 — Autenticação do Administrador}
% ─────────────────────────────────────────────────────────────
...
submete as credenciais ao endpoint \path{POST /auth/login}. O \textit{backend}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_3a.pdf}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_3b.pdf}
...
\texttt{AuthContext} invoca \path{authService.logout()}, que envia
\path{POST /auth/logout} ao \textit{backend} para marcar a sessão como inativa
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/image55_seq_3c.pdf}}
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 4 — Recuperação de Senha}
% ─────────────────────────────────────────────────────────────
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_4a.pdf}
...
\path{GET /auth/validar-token-reset/:token}, passando o token como parâmetro de
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_4b.pdf}}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_4c.pdf}
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 5 — Dashboard e Monitoramento em Tempo Real}
% ─────────────────────────────────────────────────────────────
...
\texttt{Promise.all()} — estatísticas do sensor via \path{GET /sensor/estatisticas}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_5a.pdf}}
...
consulta \path{GET /health} a cada 30 segundos para verificar a disponibilidade do
\textit{backend} e do banco de dados. O segundo consulta \path{GET /sensor/alertas}
a cada 60 segundos para atualizar o \textit{badge} do sino.

\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_5b.pdf}}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_5c.pdf}}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_5d.pdf}}
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 6 — Tela de Sensores, Filtros e Relatórios}
% ─────────────────────────────────────────────────────────────
...
padrão exibe as leituras das últimas 24 horas via \path{GET /sensor/periodo}, com
até mil registros paginados a 15 por página na interface.

\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_6a.pdf}}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_6b.pdf}}
...
Equação~\ref{eq:frisco} com a soma do fator de risco; contexto meteorológico completo da leitura;
dados de diagnóstico do firmware extraídos do campo \texttt{dadosBrutos}; e a
recomendação operacional gerada pelo firmware.

\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_6c.pdf}}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_6d.pdf}}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_6e.pdf}}
...
% ─────────────────────────────────────────────────────────────
\paragraph*{Parte 7 — Gestão de Usuários e Central de Alertas}
% ─────────────────────────────────────────────────────────────
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_7a.pdf}}
  \caption{Seq. 7a — Listagem e Estatísticas de Usuários}
  \label{fig:seq_7a}
  \fonte{Autora, 2026.}
\end{figure}

As Figuras~\ref{fig:seq_7b1} e~\ref{fig:seq_7b2} detalham os fluxos de criação e edição de usuário. A
criação de usuário básico utiliza o endpoint público \path{POST /auth/cadastro-basico},
...
\path{POST /auth/cadastro-admin}, rota protegida pelo \textit{middleware}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textwidth, keepaspectratio]{figures/image55_seq_7b1.pdf}}
  \caption{Seq. 7b — Criação de Usuário}
  \label{fig:seq_7b1}
  \fonte{Autora, 2026.}
\end{figure}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/image55_seq_7b2.pdf}}
  \caption{Seq. 7b — Edição de Usuário}
  \label{fig:seq_7b2}
  \fonte{Autora, 2026.}
\end{figure}

As Figuras~\ref{fig:seq_7c1} e~\ref{fig:seq_7c2} apresentam os fluxos de desativação e reativação de
usuários. A desativação utiliza o verbo HTTP \path{DELETE}, porém implementa
...
modal de usuários inativos, que carrega via \path{GET /auth/usuarios-inativos}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/image55_seq_7c1.pdf}}
  \caption{Seq. 7c — Desativação de Usuários Inativos}
  \label{fig:seq_7c1}
  \fonte{Autora, 2026.}
\end{figure}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/image55_seq_7c2.pdf}}
  \caption{Seq. 7c — Reativação de Usuários Inativos}
  \label{fig:seq_7c2}
  \fonte{Autora, 2026.}
\end{figure}

As Figuras~\ref{fig:seq_7d1} e~\ref{fig:seq_7d2} ilustram o carregamento da Central de Alertas e a expansão
...
alerta, são exibidos os blocos de componentes da Equação~\ref{eq:frisco}, contexto meteorológico,
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/image55_seq_7d1.pdf}}
  \caption{Seq. 7d — Central de Alertas: Carregamento}
  \label{fig:seq_7d1}
  \fonte{Autora, 2026.}
\end{figure}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/image55_seq_7d2.pdf}}
  \caption{Seq. 7d — Central de Alertas: Expansão}
  \label{fig:seq_7d2}
  \fonte{Autora, 2026.}
\end{figure}
...
\begin{figure}[H]
  \centering
  \fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/image55_seq_7e.pdf}}
...
\begin{figure}[H]
\centering
\fbox{\includegraphics[width=\textwidth]{figures/cap4_fluxograma_f0_visao_geral.png}}
...
e F8 — simulação de nível via Serial
...
JWT (Figura~\ref{fig:fluxograma_auth}); e F9 —
...
% -------------------------------------------------------------
\paragraph*{Fluxograma do Ciclo de Análise de Risco Integrado}
...
em três faixas definidas na Tabela~\ref{tab:alertas}: fator de risco
...
% -------------------------------------------------------------
\paragraph*{Fluxograma do Protocolo de Ruptura e Desescalada}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]{figures/cap4_fluxograma_f3_protocolo_ruptura.png}
...
% -------------------------------------------------------------
\paragraph*{Fluxograma do Loop Principal de Orquestração}
...
% -------------------------------------------------------------
\paragraph*{Fluxograma do Cálculo do Índice de Confiabilidade}
...
% -------------------------------------------------------------
\paragraph*{Fluxograma de Recebimento e Persistência de Leitura}
...
\path{POST /api/sensor/dados} enviada pelo \textit{firmware} do
ESP8266.
...
A rota \path{POST /api/sensor/dados} é a única rota do sistema
...
% -------------------------------------------------------------
\paragraph*{Fluxograma de Autenticação JWT e Validação de Sessão}
...
% -------------------------------------------------------------
\paragraph*{Fluxograma de Detecção de Status do Sensor no
\textit{Frontend}}
...
\path{GET /health} do \textit{backend}. Quando a requisição
...
% -------------------------------------------------------------
\paragraph*{Fluxograma do Modo de Simulação de Nível via
Serial}
...
\centering
\fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]
    {figures/cap4_fluxograma_f8_simulacao_nivel.png}}
...
% -------------------------------------------------------------
\paragraph*{Fluxograma de Envio de Alerta em Massa}
...
\path{POST /auth/enviar-alerta}.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=\textheight, height=\textwidth, keepaspectratio, angle=-90]
    {figures/cap4_fluxograma_f9_alerta_massa.png}}
...
Arduino IDE e custo acessível. O protótipo é composto por quatro categorias de componentes
eletrônicos, descritas a seguir: o microcontrolador, o sensor de
umidade, os indicadores visuais (LEDs) e o indicador sonoro
(\textit{buzzer}).
...
A leitura deste sensor segue frequência adaptativa conforme o nível 
de risco vigente: o ADC do pino A0 é amostrado a cada 30 segundos no 
nível VERDE, a cada 10 segundos no nível AMARELO e a cada 5 segundos 
no nível VERMELHO ou durante o protocolo de ruptura, concentrando o 
consumo de processamento nos períodos de maior criticidade. 
O detalhamento completo dos cinco temporizadores do \textit{firmware} 
é apresentado na Seção~\ref{subsec:arquitetura} 
e no Fluxograma~\ref{fig:fluxograma_loop}.
...
A transmissão dos dados coletados por este circuito ao restante do sistema 
ocorre em dois estágios: primeiro, o \textit{firmware} calcula o 
fator de risco (Equação~\ref{eq:frisco}) e serializa o resultado, os dados brutos 
do sensor e o diagnóstico de \textit{hardware} em um objeto JSON; 
em seguida, esse \textit{payload} é transmitido por requisição HTTP 
ao \textit{backend}, em intervalo adaptativo por nível de risco, e 
persistido em banco de séries temporais. A arquitetura completa desse 
fluxo — módulos, formato do \textit{payload} e intervalos de envio — 
é detalhada na Seção~\ref{sec:software}, particularmente na 
Seção~\ref{subsec:arquitetura} e no caso de uso UC01.
...
Quanto ao consumo de energia, o protótipo não incorpora sensor de 
corrente ou de tensão dedicado à medição em tempo real, uma vez que 
o dispositivo foi projetado para alimentação contínua via 
fonte externa/USB (5,V a 9,V), e não por bateria — razão pela qual 
não foi implementado mecanismo de alerta de substituição de bateria. 
Uma estimativa teórica pode, contudo, ser obtida a partir das correntes 
máximas informadas nos \textit{datasheets} dos componentes (Tabela~\ref{tab:specs}): 
o pior caso ocorre no nível VERMELHO, quando o ESP8266 opera com Wi-Fi ativo (até 200,mA), 
o LED vermelho está aceso (até 15,mA) e o \textit{buzzer} está ativo (até 40,mA), 
resultando em corrente combinada de aproximadamente 255,mA em 3,3–5,V — da ordem de 1,3,W. 
Essa estimativa é apresentada como referência de dimensionamento, não como medição instrumentada.
...
Como complemento ao protótipo físico descrito nas
Seções~\ref{subsec:esp8266} a~\ref{subsec:circuito}, o sistema foi
também reproduzido em ambiente de emulação virtual por meio da
plataforma Wokwi, acessada diretamente pela extensão oficial do Wokwi
para o Visual Studio Code. A escolha do Wokwi em detrimento de
alternativas como o TinkerCad Circuits decorreu de uma limitação
técnica desta última: o TinkerCad não emula conectividade Wi-Fi nem
requisições HTTP/HTTPS para microcontroladores da família ESP, o que
inviabiliza a simulação de qualquer parte do sistema que dependa de
comunicação em rede --- exatamente o núcleo funcional deste trabalho
(consulta às APIs BNDMET e OpenWeatherMap e envio de dados ao
\textit{backend}). O Wokwi, por sua vez, disponibiliza uma rede
virtual (\texttt{Wokwi-GUEST}) com \textit{gateway} de saída real
para a internet, permitindo que o \textit{firmware} emulado realize
as mesmas requisições HTTPS do protótipo físico.

O projeto de emulação é definido por dois arquivos: o
\texttt{diagram.json}, que descreve os componentes do circuito virtual
e suas conexões, e o \textit{firmware} adaptado (arquivo
\texttt{sketch.ino}), executado sobre um microcontrolador ESP32
DevKit --- em vez do ESP8266 NodeMCU do protótipo físico --- devido à
maior compatibilidade deste modelo com o simulador. A
\autoref{fig:wokwi_diagrama} apresenta o diagrama de conexões
resultante.

\begin{figure}[H]
\centering
\fbox{\includegraphics[width=0.9\textwidth]{figures/cap5_wokwi_diagrama.png}}
\caption{Diagrama de circuito do ambiente de emulação Wokwi.}
\label{fig:wokwi_diagrama}
\fonte{Autora, 2026.}
\end{figure}

Duas adaptações foram necessárias para portar a lógica do
\textit{firmware} ao ambiente simulado. Primeiro, o sensor higrômetro
resistivo FC-28 foi substituído por um potenciômetro conectado ao pino
GPIO34, cuja leitura analógica --- linear, no intervalo
de 0 a 4095 --- permite ajustar manualmente e com precisão o valor de
umidade do solo simulado. Segundo, a rede \texttt{Wokwi-GUEST} foi
configurada com o endereço IP do \textit{gateway} apontando para o
\textit{host} local, viabilizando que o dispositivo virtual enviasse
seus dados ao mesmo \textit{backend} Node.js utilizado pelo protótipo
físico, por meio do \textit{endpoint} \texttt{POST
/api/sensor/dados} descrito na Seção~\ref{subsec:arquitetura}.

O ambiente de emulação foi empregado como estratégia complementar de
validação, permitindo reproduzir de forma determinística e repetível
os três níveis de alerta e a condição de ruptura --- por meio do
ajuste direto do potenciômetro virtual --- sem depender da ocorrência
natural das condições geotécnicas correspondentes nem do desgaste do
sensor físico. Os resultados obtidos nesse ambiente são apresentados
na Seção~\ref{sec:wokwi-resultados} do
...
A validação do sistema foi conduzida de ponta a ponta, cobrindo o
dispositivo embarcado, o \textit{backend} e a interface \textit{web},
com o objetivo de verificar tanto os fluxos de sucesso quanto os
cenários alternativos e de falha previstos no modelo de casos de uso
(\autoref{subsubsec:casos-uso}). A estratégia de testes combinou
quatro abordagens complementares.

A primeira consistiu na inspeção do monitor serial do \textit{firmware}
durante a operação contínua do protótipo, permitindo observar a
sequência de inicialização, os ciclos de leitura do sensor, as
consultas às APIs externas e o cálculo do fator de risco em tempo
real. A segunda utilizou os comandos de simulação de nível
(\texttt{alerta verde}, \texttt{alerta amarelo} e
\texttt{alerta vermelho}), descritos na
\autoref{subsubsec:fluxograma-simulacao}, para reproduzir de forma
controlada e repetível os três níveis de alerta e o comportamento dos
indicadores físicos e da interface \textit{web}, sem depender da
ocorrência natural das condições geotécnicas e meteorológicas
correspondentes. A terceira consistiu na indução deliberada de
cenários de falha --- desconexão do sensor, indisponibilidade das
APIs BNDMET e OpenWeatherMap, queda de conectividade Wi-Fi e de
comunicação com o \textit{backend} --- para verificar se os mecanismos
de tolerância a falhas e os descontos no índice de confiabilidade
(\autoref{subsubsec:fluxograma-confiabilidade}) respondiam conforme o
projetado.

A quarta abordagem utilizou o ambiente de emulação virtual Wokwi,
descrito na Seção~\ref{subsec:wokwi}, executado por meio da extensão
oficial do Wokwi para o Visual Studio Code. Nesta etapa, o sensor
higrômetro físico foi substituído por um potenciômetro virtual
(\autoref{fig:wokwi_diagrama}), cujo valor foi ajustado manualmente
para reproduzir, de forma determinística, cada uma das três faixas de
alerta definidas na Tabela~\ref{tab:alertas} e o limiar de ruptura de
30\%. Diferentemente da segunda abordagem --- que injeta valores
predefinidos diretamente nas estruturas do \textit{firmware} por meio
de comandos Serial ---, o ajuste do potenciômetro no ambiente Wokwi
percorre o fluxo completo de leitura do sensor
(\autoref{fig:fluxograma_loop}), da conversão do valor bruto do ADC
até o cálculo da Equação~\ref{eq:frisco}, aproximando-se mais fielmente
do comportamento do protótipo físico em campo. Essa abordagem foi
particularmente útil para validar a integração com as APIs BNDMET e
OpenWeatherMap a partir de um ambiente de execução distinto do
\textit{hardware} físico, confirmando que a lógica de aquisição,
cálculo de risco e comunicação com o \textit{backend} é independente
do dispositivo físico ou virtual que a executa.