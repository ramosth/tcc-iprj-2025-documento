...
\usepackage[
    alf,                    % estilo alfabético    
    bibjustif,              % justifica as referências
    abnt-etal-cite=2,       % “et al.” a partir de 2 autores nas citações
    abnt-etal-list=3,       % até 3 nomes nas entradas da bibliografia
    abnt-full-initials=no,  % iniciais mínimas nos nomes
    abnt-emphasize=italic,  % títulos de obras em itálico
    abnt-repeated-author=no,% não repete sobrenome em entradas subsequentes
    abnt-upper-cite=no      % DESATIVA caixa alta nas citações
]{abntex2cite}
\usepackage{hyperref}
...
% Titulo do trabalho
\titulo{A engenharia da computação na prevenção de acidentes em barragens de rejeitos} %Título do trabalho
\newcommand{\englishTitle}{The computer engineering in preventing tailings dam accidents} %Título do trabalho em inglês
\data{\today}			%Ano de publicação
\local{Nova Friburgo}	%Local

%% Membros da banca
\newcommand{\membrobancaA}{Prof. Dr. Bernardo Sotto-Maior Peralva}
\newcommand{\intituicaoBancaA}{Instituto Politécnico - UERJ}

\newcommand{\membrobancaB}{Prof. Dra. Sílvia Cristina Dias Pinto}
